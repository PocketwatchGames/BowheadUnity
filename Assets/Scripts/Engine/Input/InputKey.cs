...
public static class InputKeyExtensions {
	...
	public static bool IsModifierKey(this InputKey key) {
		if ((key == InputKey.LeftAlt) ||
			...
			(key == InputKey.LeftCommand) ||
			(key == InputKey.LeftWindows) ||
			(key == InputKey.RightWindows) ||
			(key == InputKey.RightApple)) {
			return true;
		}
		return false;
	}

	public static string GetLongName(this InputKey key) {
		int intVal = (int)key;

		if ((intVal > 0x20) && (intVal < 0x7f)) {
			// single character ascii.
			return ((char)intVal).ToString().ToUpper();
		} else if ((intVal >= (int)InputKey.F1) && (intVal <= (int)InputKey.F15)) {
			return key.ToString();
		}

		switch (key) {
			case InputKey.ScrollWheelUp:
			return "MWHEEL-UP";
			case InputKey.ScrollWheelDown:
			return "MWHEEL-DWN";
			case InputKey.Tab:
			return "TAB";
			case InputKey.Clear:
			...
			return "ALT";
			case InputKey.RightApple:
			return "RAPPLE";
			case InputKey.LeftCommand:
			return "CMD";
			case InputKey.Help:
			return "HELP";
			case InputKey.Print:
			return "PRINT";
			case InputKey.SysReq:
			return "SYS";
			case InputKey.Break:
			return "BREAK";
		}
	...
	public static string GetShortName(this InputKey key) {
		int intVal = (int)key;
