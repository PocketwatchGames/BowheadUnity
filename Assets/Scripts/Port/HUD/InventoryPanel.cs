    ...
    public class InventoryPanel : MonoBehaviour {
        ...
        private void Rebuild() {
            for (int slot = 0; slot <= (int)Player.InventorySlot.RIGHT_HAND; slot++) {
                ...
                s.SetItem(_player.inventory[slot]);
            }
        }
        ...
        private void Update() {
            ...
            if (Input.GetButton("Use")) {
                ...
                if (Input.GetButtonUp("Use")) {
                    var item = _player.inventory[inventorySelected];
                    if (item != null) {
        ...
        void SelectPreviousInventory() {
            if (dropTimer >= dropTime) {
                ...
                for (int i = 0; i < Player.MAX_INVENTORY_SIZE; i++) {
                    ...
                    if (_player.inventory[inventorySelected] != null) {
                        break;
                    }
                }
                dropTimer = 0;
        ...
        private void RearrangeLeft() {
            var curItem = _player.inventory[inventorySelected];
            if (curItem != null) {
                Pack pack;
                if ((pack = curItem as Pack) != null) {
                    int newSlot = inventorySelected - 1;
                    while (newSlot >= 0 && !(_player.inventory[newSlot] is Pack)) {
                        newSlot--;
                    }
                    if (newSlot >= 0) {
                        List<Item> newInventory = new List<Item>();
                        for (int i = 0; i < newSlot; i++) {
                            newInventory.Add(_player.inventory[i]);
                        }
                        for (int i = 0; i < pack.Data.slots + 1; i++) {
                            newInventory.Add(_player.inventory[i + inventorySelected]);
                        }
                    ...
                    for (; newSlot > (int)Player.InventorySlot.PACK; newSlot--) {
                        var itemInNewSlot = _player.inventory[newSlot];
                        Pack p2;
                        if (itemInNewSlot is Pack) {
                            continue;
                        }
                        _player.inventory[inventorySelected] = itemInNewSlot;
                        _player.inventory[newSlot] = curItem;
                        inventorySelected = newSlot;
                        return;
        ...
        private void RearrangeRight() {
            ...
            if (curItem != null) {
                ...
                if ((pack = curItem as Pack) != null) {
                    int newSlot = inventorySelected + 1;
                    while (newSlot < Player.MAX_INVENTORY_SIZE && !(_player.inventory[newSlot] is Pack)) {
                        newSlot++;
                    }
                    if (newSlot < Player.MAX_INVENTORY_SIZE) {
                        ...
                        Pack p2 = _player.inventory[newSlot] as Pack;
                    ...
                    for (lastPackSlot = (int)Player.InventorySlot.PACK; lastPackSlot < Player.MAX_INVENTORY_SIZE; lastPackSlot++) {
                        var item = _player.inventory[lastPackSlot];
                    ...
                    if (oldSlot < (int)Player.InventorySlot.PACK) {
                        ...
                        for (; emptySlot < lastPackSlot; emptySlot++) {
                            if (_player.inventory[emptySlot] == null) {
                                break;
                            }
                        }
                        int lastEmptySlot = emptySlot;
                        for (int curSlot = emptySlot - 1; curSlot > (int)Player.InventorySlot.PACK; curSlot--) {
                            if (_player.inventory[curSlot] is Pack) {
                                continue;
                            }
                            _player.inventory[lastEmptySlot] = _player.inventory[curSlot];
                            lastEmptySlot = curSlot;
                        }
                        inventorySelected = (int)Player.InventorySlot.PACK + 1;
                        _player.inventory[(int)Player.InventorySlot.PACK + 1] = curItem;
                        _player.inventory[oldSlot] = null;
                    }
                    else {
                        for (; newSlot <= lastPackSlot; newSlot++) {
                            ...
                            _player.inventory[oldSlot] = itemInNewSlot;
                            _player.inventory[newSlot] = curItem;
                            inventorySelected = newSlot;
                            return;
