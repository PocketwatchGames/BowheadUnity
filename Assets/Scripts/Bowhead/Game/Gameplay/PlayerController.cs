...
namespace Bowhead.Actors {
	...
	public abstract class PlayerController : Actor, ActorWithTeam, MatchStateEventReceiver {
		...
		public static List<T> GetOverlapTargets<T>(World world, Team team, T initial, Vector3 initialPos, float radius, EFriendlyFire friendlyFire, bool allTypes) where T : DamageableActor {
			List<T> targets = new List<T>();
			List<Vector3> queue = new List<Vector3>();

			FogOfWarController fogOfWar = null;
			var svWorld = world as Server.ServerWorld;
			if (svWorld != null) {
				fogOfWar = svWorld.fogOfWar;
			}
			...
			if (svWorld != null) {
				if (friendlyFire == EFriendlyFire.Enemies) {
#if NO_FRIENDLIES
					layerMask = Layers.AllTeamsMask;
#else
					layerMask = Layers.AllTeamsMask & ~Layers.GetTeamLayerMask(team.teamNumber);
#endif
				} else {
					layerMask = Layers.GetTeamLayerMask(team.teamNumber);
				}
			} else {
				layerMask = Layers.SelectionMask;
			}
			...
			while (queue.Count > 0) {
				...
				var colliders = Physics.OverlapSphere(q, radius, layerMask);
				for (int i = 0; i < colliders.Length; ++i) {
					var tag = colliders[i].GetComponentInParent<ActorReference>();
					var u = (tag != null) ? (svWorld != null) ? (tag.serverActor as T) : (tag.clientActor as T) : null;
					if ((u != null) && !(u.pendingKill || u.dead) && (allTypes || (initial == null) || ReferenceEquals(u.metaClass, initial.metaClass)) && (u is TargetableActor) && !targets.Contains(u) && u.CheckFriendlyFire(friendlyFire, team) && ((fogOfWar == null) || fogOfWar.CanBeSeenByTeam(team, u))) {
						targets.Add(u);
						queue.Add(u.go.transform.position);
					}
				}
			}

			targets.Sort((a, b) => (initial.go.transform.position-a.go.transform.position).sqrMagnitude.CompareTo((initial.go.transform.position-b.go.transform.position).sqrMagnitude));

			return targets;
		}

		static int GetClosestTargetIndex<T>(IList<T> targets, Vector3 pos) where T: DamageableActor{
			int bestIdx = -1;
			float bestDist = float.MaxValue;

			for (int i = 0; i < targets.Count; ++i) {
				var t = targets[i];
				var d = (t.go.transform.position - pos).sqrMagnitude;
